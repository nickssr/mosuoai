...
本文评测主要基于公开文档、GitHub 数据和社区反馈，各框架的性能数据存在一定差异。整体来看：

- **延迟**：CrewAI 由于轻量级抽象，单步执行延迟最低；LangGraph 因检查点机制略高
- **吞吐量**：Mastra 在 TypeScript 生态中通过 Vercel Edge Runtime 实现较高并发；LangGraph 通过 LangSmith 可优化大批量调用
- **内存占用**：OpenAI Agents SDK 最轻量，适合资源受限环境

实际性能需根据具体业务场景和并发规模进行实测，以上为定性参考。
...
本文评测的五大框架（LangGraph、CrewAI、Mastra、AutoGen、OpenAgents）均为开源项目，核心功能免费使用。企业在评估成本时需考虑：

- **团队学习成本**：LangGraph 学习曲线最陡，CrewAI 最平缓
- **运维成本**：自托管需要服务器和运维投入
- **商业支持**：部分框架提供商业支持版本（如 LangChain Enterprise）

**结论**：开源框架本身无直接定价，但需评估隐性成本（学习、运维、迁移风险）。
