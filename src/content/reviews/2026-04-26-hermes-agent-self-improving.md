...
消息平台目前仅支持 5 个（vs OpenClaw 的 20+），接入微信、企业微信、QQ 等平台的需求无法满足。
...
Hermes Agent 的自改进机制在每次任务完成后会触发五阶段反思循环（记忆筛选→技能创建→技能优化→FTS5检索→用户建模），这会带来额外的延迟开销。根据社区反馈，单次任务执行额外耗时约 200-500ms，具体取决于任务复杂度和记忆库大小。

Python 技术栈在并发场景下受 GIL 限制，多 Agent 并发场景性能弱于 Node.js 的 OpenClaw。v0.7.0 版本仍为 pre-1.0，生产环境部署建议做好性能监控。

## 定价分析

Hermes Agent 是开源项目（Nous Research），核心功能免费使用。企业如需商业支持或托管版本，需联系 Nous Research 获取定制报价。OpenClaw 同样采用开源+商业支持的双轨模式。两者均为自托管方案，无直接云端订阅费用。
