...
Hermes Agent 是由开源 AI 实验室 Nous Research 推出的 AI Agent 项目，2026 年 4 月发布 v0.9.0 版本（代号「everywhere release」），短短两个月在 GitHub 获得 27,000+ 星。
...
按 `Alt+Enter` 或 `Ctrl+J` 添加新行，适合粘贴代码或写长 prompt。

## 第五步：连接消息平台（可选）

如果你想让 Hermes 成为 Telegram、Discord 或 Slack 机器人：
...
```
用户完成任务 → 记忆筛选 → 创建技能 → 优化技能 → FTS5 检索 → 用户建模
```
...
如果你在找 OpenClaw 的替代品，或想体验「会成长的 AI Agent」，Hermes 值得一试。
